    ...
    response: config => {
      ...
      groupUser.forEach(x => {
        if (!result.find((r => r.id == x.group_id && r.tag == 'group'))) {
          ...
          result.push({ id: `g_${x.group_id}`, name: groupTemp.label, tag: 'group', parentId: null })
        }
        if (!result.find((r => r.id == x.user_id && r.tag == 'user'))) {
          let userTemp = user.filter(u => u.id == x.user_id)[0]
          result.push({ id: `u_${x.user_id}`, name: userTemp.name, tag: 'user', parentId: `g_${x.group_id}`})
        }
      })
      return {
        code: 20000,
